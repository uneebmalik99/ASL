...
renderpaymentHistoryContent = ({ item }) => {
    return <Elavation
        elevation={2}
        style={{ borderRadius:15,borderColor:'black',borderWidth:1, width: deviceWidth * 0.45,  flexDirection: 'column', marginBottom: 5, backgroundColor: 'white', marginRight: 10, marginLeft: 10, marginTop: 4 }}
    >
        <TouchableOpacity style={{ flex: 1, justifyContent: 'space-between', paddingTop: 5, paddingBottom: 5, paddingLeft: 10 }}
        >
...
callingPaymentHostoryAPI =() => {

    fetch(AppUrlCollection.PAYMENT_HISTORY+'page=' + this.state.allPagination, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'authkey': AppConstance.USER_INFO.USER_TOKEN
